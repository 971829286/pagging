...
CREATE TABLE city(
  cid int PRIMARY KEY,
  pid int REFERENCES province(pid),
...
CREATE VIEW view_district AS
  ...
  FROM (`employee`.`district` `d`
    JOIN (SELECT
          ...
          FROM (`employee`.`city` `c`
            JOIN `employee`.`province` `p` ON ((`c`.`pid` = `p`.`pid`)))) `t` ON ((`d`.`cid` = `t`.`cid`)));
			
SELECT did,dname,postcode,cid,cname,areacode,pid,pname,orderid
FROM district d 
LEFT JOIN
(SELECT c.cid,c.cname,c.areacode,p.pid,p.pname,p.orderid
FROM city c 
JOIN province p 
